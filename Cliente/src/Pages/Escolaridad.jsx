...
const Escolaridad = () => {
  ...
  const add = () => {
    ...
    Axios.post(`${API_BASE_URL}/createEscolaridad`, {
      ...
      Swal.fire({
        ...
        html:
          '<i>La escolaridad <strong>' + Escolaridad_Nombre + '</strong> ha sido registrada.</i>',
        icon: 'success',
        timer: 3000,
      });
    });
  ...
  const getLista = async () => {
    try {
      const response = await authFetch(`${API_BASE_URL}/obtenerEscolaridad`);
  ...
  const actualizar = () => {
    ...
    Swal.fire({
      ...
      html:
        '<i>La escolaridad <strong>' + Escolaridad_Nombre + '</strong> ha sido actualizada.</i>',
      icon: 'success',
      timer: 3000,
    });
  };
  ...
  const eliminar = (Escolaridad_Id) => {
    Swal.fire({
      ...
      html: '<i>¿Realmente desea eliminar <strong>' + Escolaridad_Nombre + '</strong>?</i>',
      icon: 'warning',
      ...
      if (res.isConfirmed) {
        Axios.delete(`${API_BASE_URL}/deleteEscolaridad/${Escolaridad_Id}`).then(() => {
          getLista();
          limpiarDatos();
        });
